...
int main() {
	...
	VkDescriptorSetLayoutBinding set_layout_textures[] = {
		{
			...
			2, // number of resources with this layout binding
	...
	VkDescriptorSetLayout layouts[] = {
		...
		descriptor_set_layout_textures	// set 1
		// set 3
		// usw
	};
	...
	read_file("../../BindlessTextures/shaders/vert.spv", &vertex_byte_code, &vertex_code_size);
	uint8_t* fragment_byte_code = 0;
	int fragment_code_size;
	read_file("../../BindlessTextures/shaders/frag.spv", &fragment_byte_code, &fragment_code_size);
	...
	Image plane_image  = load_image("../../assets/textures/stonepath/albedo.jpg");
	
	// Create actual textures (that sit on the GPU)
	VkalTexture knight_texture = vkal_create_texture(0, knight_image.data, knight_image.width, knight_image.height, 4, 0,
	...
	while (!glfwWindowShouldClose(window)) {
...

		{
			...
			uint32_t id1 = 0;
			vkCmdPushConstants(vkal_info->default_command_buffers[image_id], pipeline_layout, VK_SHADER_STAGE_FRAGMENT_BIT, 0, sizeof(uint32_t), &id1);
			...
			uint32_t id2 = 1;
			vkCmdPushConstants(vkal_info->default_command_buffers[image_id], pipeline_layout, VK_SHADER_STAGE_FRAGMENT_BIT, 0, sizeof(uint32_t), &id2);
